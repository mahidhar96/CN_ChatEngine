        ...
        for (i = 0; i <= fdmax; i++) 
        {
            if (FD_ISSET(i, & read_fds)) 
            {
                ...
                else 
                {
                    ...
                    memset(&message,'\0',sizeof(buffer2));
                    nbytes = recv(i, (struct buffed*)&message, sizeof(buffer2), 0);
                    recv(i, (struct buffed*)&trash, sizeof(buffer2), 0);
                    if (nbytes <= 0) 
                    {
                    ...
                    else 
                    {
                        ...
                        for (j = 0; j <= fdmax; j++) 
                        {
                            ...
                            {
                                if(strcmp(message.message,"(}")!=0)//received message
                                {
                                ...
                                else //if(message.option==0)//received request to read message
                                {
                                	...
                                	message.option=smsg;
                                	sprintf(message.name,"%d",smsg);
                                	printf("%s %s %d\n",message.message,message.name,message.option);
                                	send(j, (struct buffed*)&message, sizeof(buffer2), 0);
